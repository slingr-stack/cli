    ...
    private async generateDataset(datasource: string, dataset: string, flags: any): Promise<void> {
        ...
        try {
            ...
            if (flags['generate-prompt']) {
                ...
                await this.openCopilotWithPrompt(prompt, datasource, dataset)
            }
...
*Showing 2 of ${result.records.length} generated records*`
     ...
     */
    private async openCopilotWithPrompt(prompt: string, datasource: string, dataset: string): Promise<void> {
        try {
            ...
            const tempFile = path.join(tempDir, `${datasource}-${dataset}-instructions-${timestamp}.md`)
